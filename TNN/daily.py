...
def send_telegram_report(message):
    ...
    MAX_LENGTH = 4000  # 預留一點緩衝，Telegram 上限為 4096
    ...
    for i, msg in enumerate(messages):
        payload = {
            'chat_id': CHAT_ID,
            'text': f"{msg}\n\n(第 {i+1}/{len(messages)} 頁)" if len(messages) > 1 else msg
        }
        try:
            resp = requests.post(url, json=payload, timeout=15)
            if resp.status_code != 200:
                print(f"⚠️ 第 {i+1} 段發送報錯: {resp.text}")
            resp.raise_for_status()
            # 避免觸發 Telegram 的頻率限制 (Flood limits)
            if len(messages) > 1:
                time.sleep(0.5) 
        except Exception as e:
    ...
    if len(messages) > 0:
        print(f"✅ 已處理完畢，共發送 {len(messages)} 則訊息。")
...
if __name__ == "__main__":
    print(f"🔹 台南充電槍監控啟動 (設定每日 09:00 執行報表)")
