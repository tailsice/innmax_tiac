...
def save_message_to_csv(entries):
    ...
    filename = f"ev_status_{month_str}_TAO.csv"
...
def job():
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 檢查完成，目前離線數: {len(offline_connectors)}")
        
    except Exception as e:
        error_msg = f"❌ API 或檢查錯誤：{str(e)}"
        print(error_msg)
        # 可視需求發送 Telegram 報錯
